...
/// <summary>
/// Decodificador de FT2 (Franke-Taylor 2 — experimental) que implementa <see cref="IDecodificadorDigital"/>.
/// Pipeline de procesamiento:
/// 1. Recibe bloques de audio PCM de 16 bits.
/// 2. Acumula muestras en un buffer circular de 6 segundos.
/// 3. Cuando el buffer esta lleno, analiza cada simbolo (0.04s) usando filtros Goertzel
///    para los 4 tonos posibles, espaciados 6.25 Hz.
/// 4. FT2 es similar a FT8 pero con ventana mas corta (6s vs 12.64s), disenado para
///    contactos rapidos en condiciones de senal debil.
/// </summary>
public sealed class DecodificadorFt2 : IDecodificadorDigital
{
    private readonly ConfiguracionFt2 _configuracion;
    ...
    /// <inheritdoc />
    public ModoOperacion Modo => ModoOperacion.FT2;
    ...
    /// <summary>
    /// Crea una nueva instancia del decodificador FT2.
    /// </summary>
    /// <param name="configuracion">Configuracion del decodificador. Si es null se usa la configuracion por defecto.</param>
    public DecodificadorFt2(ConfiguracionFt2? configuracion = null)
    {
        _configuracion = configuracion ?? new ConfiguracionFt2();

        int tamanoBuffer = (int)(_configuracion.TasaDeMuestreo * _configuracion.DuracionVentanaSegundos);
    ...
    /// <summary>
    /// Decodifica el buffer de audio completo analizando los tonos de cada simbolo FT2.
    /// Cada simbolo dura 0.04 segundos.
    /// </summary>
    ...
    private List<MensajeDecodificado> DecodificarBuffer(float[] buffer, DateTimeOffset marcaDeTiempo)
    {
        ...
        if (haySimbolosValidos)
        {
            ...
            MensajeDecodificado mensaje = new(
                ...
                snr: 0,
                deltaTiempo: 0.0,
                modo: ModoOperacion.FT2,
                texto: $"FT2 simbolos: [{textoSimbolos}]");
