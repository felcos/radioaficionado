...
/// <summary>
/// Decodificador de JT9 (9-FSK) que implementa <see cref="IDecodificadorDigital"/>.
/// Pipeline de procesamiento:
/// 1. Recibe bloques de audio PCM de 16 bits.
/// 2. Acumula muestras en un buffer circular de ~49 segundos.
/// 3. Cuando el buffer esta lleno, analiza cada simbolo (~0.576s) usando filtros Goertzel
///    para los 9 tonos posibles, espaciados 1.7361 Hz.
...
/// </summary>
public sealed class DecodificadorJt9 : IDecodificadorDigital
{
    private readonly ConfiguracionJt9 _configuracion;
    ...
    /// <inheritdoc />
    public ModoOperacion Modo => ModoOperacion.JT9;
    ...
    /// <summary>
    /// Crea una nueva instancia del decodificador JT9.
    /// </summary>
    /// <param name="configuracion">Configuracion del decodificador. Si es null se usa la configuracion por defecto.</param>
    public DecodificadorJt9(ConfiguracionJt9? configuracion = null)
    {
        _configuracion = configuracion ?? new ConfiguracionJt9();
    ...
    /// <summary>
    /// Decodifica el buffer de audio completo analizando los tonos de cada simbolo JT9.
    /// </summary>
    ...
    private List<MensajeDecodificado> DecodificarBuffer(float[] buffer, DateTimeOffset marcaDeTiempo)
    {
        ...
        if (haySimbolosValidos)
        {
            ...
            MensajeDecodificado mensaje = new(
                ...
                snr: 0,
                deltaTiempo: 0.0,
                modo: ModoOperacion.JT9,
                texto: $"JT9 simbolos: [{textoSimbolos}]");
    ...
    /// <summary>
    /// Analiza los 9 tonos posibles en un segmento de audio usando el algoritmo de Goertzel
    ...
    /// <returns>Indice del tono dominante (0-8), o -1 si no se detecta ningun tono valido.</returns>
