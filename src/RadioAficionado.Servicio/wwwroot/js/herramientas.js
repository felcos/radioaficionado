...
const Herramientas = (function () {
    ...
    function inicializarCalculadoraDistancia() {
        ...
        btnCalcular.addEventListener('click', function () {
            ...
            if (!coordA || !coordB) {
                divResultado.innerHTML = '<span class="error-texto">Grid locator no valido. Usa formato de 4 o 6 caracteres (ej: IN80 o IN80dk).</span>';
                return;
            }
            ...
            divResultado.innerHTML =
                '<div class="resultado-fila"><strong>Distancia:</strong> ' + distanciaKm.toFixed(1) + ' km / ' + distanciaMillas.toFixed(1) + ' mi</div>' +
                '<div class="resultado-fila"><strong>Azimut A&rarr;B:</strong> ' + azimutAB.toFixed(1) + '&deg;</div>' +
                '<div class="resultado-fila"><strong>Azimut B&rarr;A:</strong> ' + azimutBA.toFixed(1) + '&deg;</div>' +
                '<div class="resultado-fila"><strong>Coordenadas A:</strong> ' + coordA.lat.toFixed(4) + ', ' + coordA.lng.toFixed(4) + '</div>' +
                '<div class="resultado-fila"><strong>Coordenadas B:</strong> ' + coordB.lat.toFixed(4) + ', ' + coordB.lng.toFixed(4) + '</div>';
        });
    }
    ...
    function inicializarConversorMaidenhead() {
        ...
        if (btnGridACoord && inputGrid && resultadoGrid) {
            btnGridACoord.addEventListener('click', function () {
                ...
                if (!coord) {
                    resultadoGrid.innerHTML = '<span class="error-texto">Grid no valido.</span>';
                    return;
                }
        ...
        if (btnCoordAGrid && inputLat && inputLng && resultadoCoord) {
            btnCoordAGrid.addEventListener('click', function () {
                ...
                if (isNaN(lat) || isNaN(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) {
                    resultadoCoord.innerHTML = '<span class="error-texto">Coordenadas no validas (lat: -90 a 90, lng: -180 a 180).</span>';
                    return;
                }
        ...
        if (btnMiUbicacion) {
            btnMiUbicacion.addEventListener('click', function () {
                if (!navigator.geolocation) {
                    if (resultadoCoord) {
                        resultadoCoord.innerHTML = '<span class="error-texto">Geolocalizacion no disponible en este navegador.</span>';
                    }
                    return;
                }
                navigator.geolocation.getCurrentPosition(
                    ...
                    function (error) {
                        if (resultadoCoord) {
                            resultadoCoord.innerHTML = '<span class="error-texto">Error al obtener ubicacion: ' + error.message + '</span>';
                        }
                    }
                );
            });
